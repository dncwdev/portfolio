...
GRAPH_NO_CONTEXT_MESSAGE = (
    "GraphRAG에서 관련 엔티티와 컨텍스트를 찾지 못해 규정 준수 여부를 판단하기 어렵습니다."
)

GRAPH_ANSWER_PROMPT = """
당신은 공공조달 구매규격서의 법령 준수 여부를 검토하는 GraphRAG 분석가입니다.
반드시 한국어로만 답변하세요.

아래의 그래프 커뮤니티 요약과 관련 청크만 근거로 사용하세요.
- 근거가 부족하면 단정하지 말고 '추가 확인 필요'로 판단하세요.
- 계약 이후 이행 여부가 아니라, 규격서/제안요청서에 해당 요건이 포함되었는지에만 집중하세요.
...
def retrieve_and_answer(question: str, model_name: str) -> dict[str, Any]:
    ...
    try:
        with GraphIndexer() as indexer:
            ...
            question_entities = indexer.extract_entities(normalized_question)
        ...
        return {
            "question": normalized_question,
            "answer": f"GraphRAG 처리 중 오류가 발생했습니다: {exc}",
            "contexts": [],
        }
    ...
    if not communities and not chunks:
        chunks = _fallback_vector_contexts(
            normalized_question,
            document_store,
            regulations_store,
        )
    ...
    contexts = [
        *_serialize_community_contexts(communities),
        *_serialize_chunk_contexts(chunks),
    ]
    ...
    prompt = GRAPH_ANSWER_PROMPT.format(
        question=normalized_question,
        question_entities=", ".join(entity["name"] for entity in entities[:12]) or "없음",
        graph_context=_format_graph_context(communities),
        chunk_context=_format_chunk_context(chunks),
    )
...
def _fallback_vector_contexts(
    question: str,
    document_store,
    regulations_store,
) -> list[dict[str, str]]:
    fallback_documents = [
        *document_store.similarity_search(question, k=3),
        *regulations_store.similarity_search(question, k=3),
    ]
    chunks: list[dict[str, str]] = []
    for document in fallback_documents[:6]:
        metadata = document.metadata or {}
        chunks.append(
            {
                "chunk_id": str(metadata.get("chunk_id", "fallback")),
                "source": str(metadata.get("source", "unknown")),
                "page": str(metadata.get("page", "-")),
                "source_group": str(metadata.get("source_group", "fallback")),
...
def _format_chunk_context(chunks: list[dict[str, Any]]) -> str:
    if not chunks:
        return "관련 청크 없음"
    return "\n\n".join(
        "[source={source} page={page} group={group}]\n{text}".format(
